...
func TestCasesByAssignee(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:   "OK by status and worked",
			status: "Pending,worked:true",
			setup: func() {
				pact.
					AddInteraction().
					Given("I have a pending case assigned").
					UponReceiving("A request to get my pending, unworked cases").
					WithRequest(dsl.Request{
						Method: http.MethodGet,
						Path:   dsl.String("/api/v1/assignees/47/cases"),
						Query: dsl.MapMatcher{
							"filter": dsl.String("caseType:lpa,active:true,status:Pending,worked:true"),
							"sort":   dsl.String("caseSubType:asc"),
							"page":   dsl.String("1"),
						},
						Headers: dsl.MapMatcher{
